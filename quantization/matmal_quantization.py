...
def generate_quantization_int8_constants(alpha, beta):
    #根据输入输出数据的尺度 计算 scaling 系数 和 zero-point
...
def main():
    ...
    Y_expected_prime_q_dq = dequantization(x_q=Y_expected_prime_q,
                                           s=s_Y,
                                           z=z_Y)
    Y_error_q = np.matmul(X_q,W_q) + b_q  # 直接通过量化的矩阵乘法 这种计算方法是错误的
    ...
    print("Expected Quantized Y_q from Quantized Matrix Multiplication:")m
    print(Y_q_simulated)
    print(
